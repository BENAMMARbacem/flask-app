...
class FeatureEngineering:
    ...
    def normalise_chunk_amplitude(self, chunk_path, output_folder):
        ...
        normalised_chunk_path = os.path.join(output_folder, audio_filename)
        os.makedirs(output_folder, exist_ok=True)
    ...
    def normalise_chunk_length(self, chunk_path, output_folder, length):
        ...
        audio_filename = os.path.basename(chunk_path)
        os.makedirs(output_folder, exist_ok=True)
        output_chunk_path = os.path.join(output_folder, audio_filename)
    ...
    def mfcc_and_image_combination(self, chunks_path):
        mfccs = []
        ...
        for chunk in chunks_path:
            ...
            mfcc = librosa.feature.mfcc(S=S_db_mel, sr=sr, n_mfcc=13)
            # print(mfcc.shape)
            mfcc_deriv = librosa.feature.delta(mfcc)
            # print(mfcc_deriv.shape)
            combined_mfcc = np.vstack((mfcc, mfcc_deriv))
            # print(combined_mfcc.shape)

            mfccs.append(combined_mfcc)

        return np.concatenate(mfccs, axis=1)

    def pipeline_mfcc_and_image_combination(self, input_folder):
        target = []
        cc_data_paths = []
        cd_data_paths = []
        ...
        # Process each group of chunks using mfcc_and_image_combination and collect data
        cc_data = [self.mfcc_and_image_combination(chunks_path) for chunks_path in cc_chunks_group]
        cd_data = [self.mfcc_and_image_combination(chunks_path) for chunks_path in cd_chunks_group]

        # Concatenate cc_data and cd_data into one dataset
        all_data = np.array(cc_data + cd_data)
        print(all_data.shape)
        # Create target labels: 0 for 'cc' and 1 for 'cd'
    ...
    def pipeline_combine_mfcc_image(self, input_folder):
        data_paths = [], []
        ...
        # Group audio chunks by IDs for 'cc' and 'cd'
        chunks_group = self.group_chunks_by_audio_id(data_paths).values()

        # Combine MFCC images for each group of chunks
        data = [self.combine_mfcc_images(chunks_path) for chunks_path in chunks_group]
